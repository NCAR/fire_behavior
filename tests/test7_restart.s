...
#################################################
#
# Purpose: Restart regression for the real test7 fire spread case.
#
#################################################
#
set -uo pipefail
...
file_base_namelist="$test_dir/test7/namelist.fire"
file_wrf="$test_dir/test7/wrf.nc"
file_geo="$test_dir/test7/geo_em.d01.nc"
work_dir="$test_dir/restart_test7"

continuous_dir="$work_dir/continuous"
restart_write_dir="$work_dir/restart_write"
restart_read_dir="$work_dir/restart_read"

continuous_output="$continuous_dir/test7_continuous_output.txt"
restart_read_output="$restart_read_dir/test7_restart_read_output.txt"
...
run_model "$restart_write_dir" "$restart_write_dir/test7_restart_write_output.txt"
exe_status=$?
if [ "$exe_status" -ne 0 ]; then
  echo "TEST 7 RESTART:"
  echo "  FAIL: restart write run failed"
  tail -n 40 "$restart_write_dir/test7_restart_write_output.txt"
  exit 1
fi
...
compare_metric 'Test7_restart.1' 'Fire area' 6
compare_metric 'Test7_restart.2' 'Heat output' 6
compare_metric 'Test7_restart.3' 'Latent heat output' 7
compare_metric 'Test7_restart.4' 'Max heat flux' 7
compare_metric 'Test7_restart.5' 'Max latent heat flux' 8
